class YBusCreator
	def initialize(num_of_buses, num_of_lines, data)
		@from_bus_list = []
		@to_bus_list = []
		@resistance_list = []
		@reactance_list = []
		...
		num_of_lines.times do |i|
			...
			if (@reactance_list[i] == 0)
				@y[i] = [(@resistance_list[i]/deno), (@reactance_list[i]/deno)]
			else
				@y[i] = [(@resistance_list[i]/deno), (-1*@reactance_list[i]/deno)]
			end
		end
	end

	def return_from_bus_list
		return @from_bus_list
	end
	...
	def create_y_bus(num_of_buses, num_of_lines)
		...
		num_of_lines.times do |i|
			...
			y_bus[@from_bus_list[i]][@to_bus_list[i]][0] = y_bus[@from_bus_list[i]][@to_bus_list[i]][0] - @y[i][0]/@tap_setting[i]
			y_bus[@from_bus_list[i]][@to_bus_list[i]][1] = y_bus[@from_bus_list[i]][@to_bus_list[i]][1] - @y[i][1]/@tap_setting[i]
